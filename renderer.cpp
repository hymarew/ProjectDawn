...
// ============================================================
// スキニング(ボーン変形)モデル用の頂点シェーダー作成
// VERTEX_3D_SKIN(Position/Normal/Color/TexCoord + BoneIndices/BoneWeights)に対応する入力レイアウトを使う。
// 既存のCreateVertexShader(VERTEX_3D専用)には手を入れず、Instancing用と同じく専用関数として追加する。
// ============================================================
void Renderer::CreateVertexShaderSkinned(ID3D11VertexShader** VertexShader, ID3D11InputLayout** VertexLayout, const char* FileName)
{
	FILE* file;
	long int fsize;
